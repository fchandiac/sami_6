...
function CustomPagination(props) {
    ...
    let totalPaid = 0
    rows.map(row => {
        totalPaid += row[1][infoField]
    })
    let renderTotalPaid = money ? renderMoneystr(total) : totalPaid

    let totalPen = 0
    rows.map(row => {
        totalPen += row[1][infoField]
    })
    let renderTotalPen = money ? renderMoneystr(total) : totalPen
