...
function CustomPagination(props) {
    const { excelFileName, setGridApiRef, infoField, infoTitle, money, updatePaysGrid  } = props
    ...
    let totalPaid = 0
    rows.map(row => {
        totalPaid += row[1]['paid']
    })
    let renderTotalPaid = money ? renderMoneystr(totalPaid) : totalPaid

    let totalBalance = 0
    rows.map(row => {
        totalBalance += row[1]['balance']
    })
    let renderTotalBalance = money ? renderMoneystr(totalBalance) : totalBalance
    ...
    return (
        <Box sx={{ p: 1 }} width={'100%'}>
            <Stack justifyContent="space-between" direction={'row'} alignItems="center">
                ...
                <Typography color={'green'}>{ 'Pagado ' +  renderTotalPaid}</Typography>
                <Typography color={'error'}>{ 'Saldo Pendiente ' +  renderTotalBalance}</Typography>
                <Stack
                    direction="row-reverse"
                    justifyContent="space-between"
                    alignItems="center"
...
export default function AppInfoDataGrid(props) {
